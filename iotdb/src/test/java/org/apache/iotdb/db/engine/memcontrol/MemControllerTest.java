...
public class MemControllerTest {
  ...
  private void testRecordMemController() {
    ...
    for (int i = 0; i < 7; i++) {
      BasicMemController.UsageLevel level = memController.acquireUsage(dummyUser[i], 1 * GB);
    ...
    BasicMemController.UsageLevel level = memController.acquireUsage(dummyUser[0], 100 * GB);
    assertEquals(BasicMemController.UsageLevel.DANGEROUS, level);
